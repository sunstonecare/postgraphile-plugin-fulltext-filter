...
export default function PostGraphileFulltextFilterPlugin(builder) {
  ...
  builder.hook('build', (build) => {
    ...
    const tsvectorType = introspectionResultsByKind.type.find((t) => t.name.includes('tsvector'))
    if (!tsvectorType) {
  ...
  builder.hook('GraphQLObjectType:fields', (fields, build, context) => {
    ...
    if (!(isPgRowType || isPgCompoundType) || !table || table.kind !== 'class' || !pgTsvType) {
      return fields
    }

    const tableType = introspectionResultsByKind.type.filter(
      (type) => type.type === 'c' && type.namespaceId === table.namespaceId && type.classId === table.id,
    )[0]
    if (!tableType) {
        ...
        ({ addDataGenerator }) => {
          addDataGenerator(({ alias }) => ({
            pgQuery: (queryBuilder) => {
              ...
              queryBuilder.select(
                sql.fragment`ts_rank(${identifier}, to_tsquery(${sql.value(tsQueryString + ':*')}), 2)`,
                alias,
              )
            },
          }))
      ...
      tsvColumns
        ...
        .reduce((memo, attr) => {
          ...
          const findExpr = ({ queryBuilder }) => {
            ...
            return sql.fragment`ts_rank(${identifier}, to_tsquery(${sql.value(tsQueryString + ':*')}), 2)`
          }
